...
insert into member values('kkt', 'a1004', '김기태', '가산동12', 
'010-1004-1004-', '2023-09-12');
																				
--2) 도서 테이블에 다음 데이터를 추가하시오.									
--시퀀스번호	IT	스프링프레임워크	38000	8	김기태	정복사	2022-09-10		
insert into book values(bseq.nextval, 'IT', '스프링프레임워크', '38000', '8', '김기태'
, '정복사', '2022-09-10');
...
alter table member add visited int default 0; --0하고 null은 다르다									
										
--6) 회원 테이블에 이메일(email) 컬럼을 추가하시오(컬럼타입 및 제약조건 등은 본인이 판단하여 할 것.)									
alter table member add email varchar(200);--만약 필수항목이라면 not null
--처음부터 not null을 했다면 오류안날텐데 이미 앞에 다 만들어놔서 기입을 해줘야 오류안남
										
--7) 회원 테이블에 지역코드(areacode) 컬럼을 추가하시오.(컬럼타입 및 제약조건 등은 본인이 판단하여 할 것.)									
alter table member add areacod varchar(6);										
...
create table book2 as select * from book; --()해주는게 좋음										
...
select * from member2 where id like '%j%'; --조회  지우기전에 확인부터
delete from memeber2 where id like '%j%';   --삭제
										
--15) 회원(member) 테이블에서 모든 회원에 대한 포인트를 100 이 지급될 수 있도록 변경하시오.									
--update member set pt=100; --만약 1000포인트를 가지고이던 회원도 100포인트로 만들어버린다										
update member set pt=pt+100;
										
--16) 회원(member) 테이블에서 회원의 아이디가 lsh인 회원의 주소를 '도화동 27'로 변경하시오.									
update member set address='도화동27' where id='lsh'; --아이디가 lsh인 사람 										
										
--17) 회원2(member2) 테이블에서 연락처가 02인 회원에 대하여 가입일을 오늘날짜로 변경하시오.									
update member2 set reg_date=sysdate where tel like '02%';--02로 시작하는사람										
...
alter table member2 modify id varchar(12); --기본키 안지워짐 그래서 constraints쓴다.
...
insert into bookkind values('TC', '일반서적');

alter table book2 add constraint bpk2 primary key (bookid);
alter table book2 add constraint bfk2 foreign key (bookkind) reference bookkind(kindcode);
...
select distinct id,password,name from member where id not in (select id from sales); --구매한적 없는
--distinct 중복제거
...
--select * from book where pubdate like '2022%'; pubdate가 varchar이면 가능 timestamp라 안됨
--but date는 가능 varchar와 timestamp의 중간역할
										
--33) 회원(member) 테이블에서 비밀번호(password)가 5글자 이상인 회원의 아이디(id), 이름(name), 연락처(tel)을 검색하시오.									
select id, name, tel from member where length(password)>=5; --length 글자의 개수										
...
--오름차순 asc(어센딩) : 작은 것에서 부터 큰 것 순으로
--내림차순 desc(디센딩) : 큰 것에서 부터 작은 것 순으로
...
select id, bno, max(money) as "파매금액" from sales group by id, bno;										
...
--select * from book order by bookcount asc	--하나의 테이블 결과

--42) 판매(sales) 테이블에서 판매금액(money)가 큰 순으로 3위 까지인 튜플의 판매코드(sno), 
--도서코드(bno), 회원아이디(id)가 출력될 수 있도록 하시오.									
...
--44) 판매 뷰(sales_view)를 생성하되 판매(sales) 테이블을 활용하고, 판매코드(sno), 
--도서코드(bno), 아이디(id), 판매금액(money) 컬럼만 추출되어 생성되게 하시오.									
...
create view book-view as select bookking, booktitle, bookprice, pubcom from book2 where bookkind in ('IT','TC','HC');										
...
create of replace view sales_view as select * from sales where amount>=2;
										
--48) 회원 뷰(mem_view)에서 아이디(id)가 y로 끝나는 회원의 데이터를 삭제하시오.									
delete from mem_view where idlike '%y';
...
drop sequence sd_seq; --시작값은 못바꾼다
...
select name from member where id not in (select id from sales);	--구매한적 없는 회원이름
...
from book a left outer join sales b on a.bookid=b.bno 
minus select distinct a.booktitle, a.bookprice, a.author
...
left outer join sales b on a.id=b.id minus select distinct a.id, a.name 
from member a right outer join sales b on a.id=b.id;
...
create view minus_book_view as (select bookid, bookkind, booktitle, bookprice
from book minus select bookid, bookkind, booktitle, bookprice from book2);	
